    ...
    public class ManifestParser
    {
        ...
        public List<RuTubeVideoFormat> Parse()
        {
            ...
            for (int i = 0; i < matches.Count; ++i)
            {
                ...
                string encoded = WebUtility.UrlEncode(tmp); // Replaced HttpUtility with WebUtility
                if (i > 0)
                {
            ...
            for (int i = 1; i < max; i += 2)
            {
                ...
                string codecs = dict.ContainsKey("CODECS") ? WebUtility.UrlDecode(dict["CODECS"]) : null; // Replaced HttpUtility with WebUtility
